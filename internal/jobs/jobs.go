// Package jobs tracks in-flight local/relay transfers so the HTTP layer can
// stream their progress over SSE and support cancellation. State is
// in-memory only — this is a disposable local tool, not a service.
package jobs
